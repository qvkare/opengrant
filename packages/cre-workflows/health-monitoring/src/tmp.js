...
function toBase64(str) {
  ...
  for (let i2 = 0;i2 < bytes.length; i2 += 3) {
    ...
    result += i2 + 1 < bytes.length ? CHARS[(b1 & 15) << 2 | b2 >> 6] : "=";
    result += i2 + 2 < bytes.length ? CHARS[b2 & 63] : "=";
  }
  return result;
}
var fetchActiveApis = (sendRequester, apiUrl, apiKey) => {
...
var checkApisHealth = (sendRequester, apis, timeoutMs, degradedMs) => {
  const results = [];
...
var submitReport = (sendRequester, apiUrl, apiKey, report2) => {
  const resp = sendRequester.sendRequest({
    ...
    body: toBase64(JSON.stringify(report2))
  }).result();
  return ok(resp);
};
var deactivateApis = (sendRequester, apiUrl, apiKey, apiIds) => {
  const resp = sendRequester.sendRequest({
...
var onCronTrigger = (runtime2, _payload) => {
  const apiKey = runtime2.getSecret({ id: "OPENGRANT_API_KEY" }).result().value;
  const httpClient = new cre.capabilities.HTTPClient;
  const activeApis = httpClient.sendRequest(runtime2, fetchActiveApis, consensusIdenticalAggregation())(runtime2.config.apiUrl, apiKey).result();
  if (!activeApis.apis || activeApis.apis.length === 0) {
    runtime2.log("No active APIs to monitor");
  ...
  runtime2.log(`Checking health of ${activeApis.apis.length} API(s)`);
  const checks = httpClient.sendRequest(runtime2, checkApisHealth, consensusIdenticalAggregation())(activeApis.apis, runtime2.config.healthCheckTimeoutMs, runtime2.config.degradedThresholdMs).result();
  const apisToDeactivate = [];
  for (const check of checks) {
    if (check.status === "down") {
      const api = activeApis.apis.find((a) => a.id === check.apiId);
      const failures = (api?.consecutiveFailures ?? 0) + 1;
      if (failures >= runtime2.config.maxConsecutiveFailures) {
  ...
  httpClient.sendRequest(runtime2, submitReport, consensusIdenticalAggregation())(runtime2.config.apiUrl, apiKey, report2).result();
  if (apisToDeactivate.length > 0) {
    runtime2.log(`Deactivating ${apisToDeactivate.length} API(s) due to repeated failures`);
    httpClient.sendRequest(runtime2, deactivateApis, consensusIdenticalAggregation())(runtime2.config.apiUrl, apiKey, apisToDeactivate).result();
  }
  runtime2.log(`Health check complete: ${healthy} healthy, ${degraded} degraded, ${down} down`);
  return JSON.stringify({
    totalChecked: report2.totalChecked,
    healthy,
    degraded,
    down
  });
