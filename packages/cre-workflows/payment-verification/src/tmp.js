...
var verifyWithFacilitator = (sendRequester, facilitatorUrl, paymentPayload, paymentRequirement) => {
...
var onHTTPTrigger = (runtime2, payload) => {
  ...
  try {
    ...
    paymentPayload = JSON.parse(bodyStr);
  } catch {
  ...
  runtime2.log(`Verifying payment from ${paymentPayload.payload.authorization.from}`);
  const httpClient = new cre.capabilities.HTTPClient;
  const paymentRequirement = {
    scheme: "exact",
    network: runtime2.config.network,
    maxAmountRequired: paymentPayload.payload.authorization.value,
    payTo: paymentPayload.payload.authorization.to,
    asset: runtime2.config.usdcAddress
  };
  try {
    const result = httpClient.sendRequest(runtime2, verifyWithFacilitator, consensusIdenticalAggregation())(runtime2.config.facilitatorUrl, paymentPayload, paymentRequirement).result();
    if (result.success) {
      runtime2.log("Payment verified successfully");
      return JSON.stringify({
        valid: true,
        txHash: result.txHash || result.txID
      });
    }
    const errorMsg = result.errorReason || result.error || "Facilitator verification failed";
    runtime2.log(`Payment verification failed: ${errorMsg}`);
    return JSON.stringify({
      valid: false,
      error: errorMsg
    });
  } catch (err) {
    const errorMsg = err instanceof Error ? err.message : "Facilitator request failed";
    runtime2.log(`Payment verification error: ${errorMsg}`);
    return JSON.stringify({
      valid: false,
      error: errorMsg
    });
