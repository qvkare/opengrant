...
var fetchPending = (sendRequester, apiUrl, apiKey) => {
  ...
  if (!ok(resp)) {
    throw new Error(`Failed to fetch pending settlements: ${resp.statusCode}`);
  }
  return json(resp);
};
var triggerSettlement = (sendRequester, apiUrl, apiKey) => {
  const resp = sendRequester.sendRequest({
...
var onCronTrigger = (runtime2, _payload) => {
  ...
  const pending = httpClient.sendRequest(runtime2, fetchPending, consensusIdenticalAggregation())(runtime2.config.apiUrl, apiKey).result();
  if (!pending.records || pending.records.length === 0) {
    runtime2.log("No pending settlements found");
    return JSON.stringify({ settled: 0, totalAmount: "0" });
  }
  runtime2.log(`Found ${pending.records.length} pending settlement(s), triggering settlement`);
  const result = httpClient.sendRequest(runtime2, triggerSettlement, consensusIdenticalAggregation())(runtime2.config.apiUrl, apiKey).result();
  runtime2.log(`Settlement complete: ${result.processed} processed, total ${result.totalSettled}`);
